class IndividualsController < ApplicationController
  ...
  def all_by_uid          
    ...
    if params[:name_sorted] 
      session[:individual_name_sort] = !session[:individual_name_sort]         
      @individuals.sort! { |a,b| Individual.compare_names( a, b, session[:individual_name_sort] ) }
      
    elsif params[:birth_sorted] 
      session[:individual_birth_sort] = !session[:individual_birth_sort] 
      @individuals.sort! { |a,b| 
          Event.compare_dates( a.birth, b.birth, session[:individual_birth_sort] ) } 
    end
  ...
  private
    ...
    def individual_params
      params.require(:individual).permit(:name, :surname, :given, :nickname, :prefix, :suffix, :sex, :birth_id, :death_id, :uid, :parents_uid, :changed_ged, :pedigree, :tree, :gedfile, :note, :gedraw)
    end
end
